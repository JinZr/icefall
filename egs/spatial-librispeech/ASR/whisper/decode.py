...
#                                            Wei Kang)
#           2024 Yuekai Zhang
#           2024 Tsinghua University (Author: Zengrui Jin)
#
...
def save_results(
    ...
    for key, results in results_dict.items():
        ...
        results_word = []
        for res in results:
            results_word.append((res[0], res[1], res[2]))
        with open(errs_filename, "w") as f:
            wer = write_error_stats(
                f, f"{test_set_name}-{key}", results_word, enable_log=enable_log
            )
...
def main():
    parser = get_parser()
    SpatialLibriSpeechAsrDataModule.add_arguments(parser)
    ...
    spatial_librispeech = SpatialLibriSpeechAsrDataModule(args)
    test_cuts = spatial_librispeech.test_cuts()
    ...
    test_cuts = test_cuts.filter(remove_long_utt)
    test_dl = spatial_librispeech.test_dataloaders(test_cuts)
