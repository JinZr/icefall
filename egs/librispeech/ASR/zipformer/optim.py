...
class OffsetAdam(BatchedOptimizer):
    """
     OffsetAdam is like ScaledAdam but with an offset term that corresponds
     to the tensor we initialized the optimization with.  The optimizer works as ScaledAdam
     would work if we factorized each non-scalar tensor t into: (t_init * t_scale + t_offset),
     where t_init (the tensor that we initialized with) is treated as a constant, t_scale is a
     learnable scalar, and t_offset is optimized like a normal tensor in ScaledAdam.

     ScaledAdam is a variant of Adam where we scale the update of each parameter
     proportional to the norm of that parameter; and also learn the scale of the parameter,
     in log space, subject to upper and lower limits (as if we had factored each parameter as
     param = underlying_param * log_scale.exp())
    ...
    def _init_state(self, group: dict, p: Tensor, state: dict):
        ...
        if numel > 1:
            ...
            # the parameter tensor.
            # it has a shape like (batch_size, 1, 1, 1, 1)
            # TODO: we can later update this factor of 0.5
            initial_param = 0.5 * p
            param_rms = ((p - initial_param) ** 2).mean(dim=list(range(1, p.ndim)), keepdim=True).sqrt()
            state["param_rms"] = param_rms
            ...
            # version of scale_grads that is about product of grad with initial_param
            state["initial_scale_grads"] = torch.zeros(
                size_update_period, *param_rms.shape, **kwargs
            )
            state["initial_scale_exp_avg_sq"] = torch.zeros_like(param_rms)

            # we will modify the scale of "initial_param" as we train.
            state["initial_param"] = initial_param
    ...
    def _step_one_batch(
        ...
        if numel > 1:
            # Update the size/scale of p, and set param_rms
            scale_grads = state["scale_grads"]
            initial_scale_grads = state["initial_scale_grads"]
            initial_param = state["initial_param"]

            scale_grads[step % size_update_period] = (p * grad).sum(
                dim=list(range(1, p.ndim)), keepdim=True
            )
            initial_scale_grads[step % size_update_period] = (initial_param * grad).sum(
                dim=list(range(1, p.ndim)), keepdim=True
            )
            if step % size_update_period == size_update_period - 1:
                p_real = p - initial_param
                param_rms = state["param_rms"]  # shape: (batch_size, 1, 1, ..)
                param_rms.copy_(
                    (p_real ** 2).mean(dim=list(range(1, p.ndim)), keepdim=True).sqrt()
                )
                if step > 0:
                    # self._size_update() learns the overall scale on the
                    # parameter (not including the offset term), by shrinking or
                    # expanding it.
                    self._size_update(group, scale_grads - initial_scale_grads,
                                      p_real, state)
                    # self._offset_update() changes the scale of the offset
                    # "initial_param".
                    self._offset_update(group, initial_scale_grads, state)
    ...
    def _size_update(
        self, group: dict, scale_grads: Tensor, p_real: Tensor, state: dict
    ) -> None:
        """
               Called only where p.numel() > 1, this updates the scale of the parameter.
               If we imagine: pl =  underlying_param * scale.exp(), and we are doing
               gradient descent on underlying param and on scale, this function does the update
               on `scale`.
        ...
        scale_grads: a tensor of shape (size_update_period, batch_size, 1, 1,...) containing
                      grads w.r.t. the scales.
             p_real:  The parameter to update: the parameter p minus the offset term, i.e.
                     p_real = p - initial_param.
               state: The state-dict of p
        """
        param_rms = state["param_rms"]
        ...
        step = state["step"]
        size_update_period = scale_grads.shape[0]
        batch_size = scale_grads.shape[1]
        # correct beta2 for the size update period: we will have
        # faster decay at this level.
        beta2_corr = beta2**size_update_period
        ...
        delta = state["delta"]
        # the factor of (1-beta1) relates to momentum.
        delta.add_(p_real * scale_step, alpha=(1 - beta1))


    def _offset_update(
        self, group: dict, initial_scale_grads: Tensor, state: dict
    ) -> None:
        """
               Called only where p_real.numel() > 1, this updates the scale of the parameter.
               If we imagine: p_real =  underlying_param * scale.exp(), and we are doing
               gradient descent on underlying param and on scale, this function does the update
               on `scale`.

               Args:
              group: dict to look up configuration values
          initial_scale_grads: a tensor of shape (size_update_period, batch_size, 1, 1,...) containing
                     grads w.r.t. the scale on the offset.
                  p:  The parameter to update
               state: The state-dict of p
        """
        beta1, beta2 = group["betas"]
        ...
        eps = group["eps"]
        step = state["step"]
        batch_size = initial_scale_grads.shape[1]

        size_update_period = initial_scale_grads.shape[0]
        # correct beta2 for the size update period: we will have
        # faster decay at this level.
        beta2_corr = beta2**size_update_period

        initial_scale_exp_avg_sq = state["initial_scale_exp_avg_sq"]  # shape: (batch_size, 1, 1, ..)
        initial_scale_exp_avg_sq.mul_(beta2_corr).add_(
            (initial_scale_grads**2).mean(dim=0),  # mean over dim `size_update_period`
            alpha=1 - beta2_corr,
        )  # shape is (batch_size, 1, 1, ...)
        ...
        denom = initial_scale_exp_avg_sq.sqrt() + eps

        scale_step = (
            -size_lr * (bias_correction2**0.5) * initial_scale_grads.sum(dim=0) / denom
        )

        # initial_param is the offset term that we are going to scale up or down.
        initial_param = state["initial_param"]
        param_rms = (initial_param ** 2).mean(dim=list(range(1, initial_param.ndim)), keepdim=True).sqrt()

        # we don't bother to prevent initial_param from getting too small, only from getting
        # too large.
        ...
        delta = state["delta"]
        # the factor of (1-beta1) relates to momentum.
        delta.add_(initial_param * scale_step, alpha=(1 - beta1))
        # update initial_param directly, without momentum
        initial_param.mul_(1 + scale_step)
...
def _test_scaled_adam(hidden_dim: int):
    ...
    for iter in [2, 1, 0]:
        ...
        if iter == 0:
            ...
            optim = OffsetAdam(m.parameters(), lr=0.06, clipping_scale=2.0)
        ...
        for epoch in range(180):
            ...
            for n, (x, y) in enumerate(train_pairs):
                ...
                if n == 0 and epoch % 5 == 0:
                    ...
                    logging.info(
                        f"Test {iter}, epoch {epoch}, batch {n}, avg_loss {avg_loss:.4g}, lr={lr:.4e}"
        ...
        logging.info(f"Test={iter}, Time taken: {stop - start}")
