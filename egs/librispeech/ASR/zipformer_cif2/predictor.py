...
if __name__ == "__main__":
    ...
    hidden = torch.from_numpy(np.random.rand(16, 100, 512)).float()
    alphas = torch.from_numpy(np.random.rand(16, 100)).float()
    threshold = 0.5
    l, f = cif(hidden, alphas, threshold)
    print(l)
    print(f)
    print(l.shape)
    print(f.shape)

    cif_mod = Predictor(idim=512, l_order=1, r_order=1, threshold=1.0)
    l2, f2 = cif_mod(hidden, torch.randn(16, 25), mask=torch.randn(16, 100, 1))
    print(l2)
    print(f2)
    print(l2.shape)
    print(f2.shape)
