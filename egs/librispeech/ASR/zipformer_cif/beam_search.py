...
def greedy_search_batch_cif(
    ...
    # blank_id = model.decoder.blank_id
    unk_id = 1  # getattr(model, "unk_id", blank_id)
    context_size = model.decoder.context_size
    ...
    hyps = [[-1] * (context_size - 1) + [0] for _ in range(N)]
    # hyps = [[-1] * (context_size - 1) + [blank_id] for _ in range(N)]
...
def deprecated_greedy_search_batch(
    ...
    hyps = [[-1] * (context_size - 1) + [0] for _ in range(batch_size)]
