...
# It will generate data_cif/lang_bpe_xxx,
# data_cif/lang_bpe_yyy if the array contains xxx, yyy
vocab_sizes=(
  # 5000
  # 2000
  # 1000
...
if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  ...
  for vocab_size in ${vocab_sizes[@]}; do
    lang_dir=data_cif/lang_bpe_${vocab_size}
    mkdir -p $lang_dir
    # We reuse words.txt from phone based lexicon
    # so that the two can share G.pt later.
    cp data_cif/lang_phone/words.txt $lang_dir
    ...
    if [ ! -f $lang_dir/L_disambig.pt ]; then
      ./local/prepare_cif_lang_bpe.py --lang-dir $lang_dir
...
if [ $stage -le 8 ] && [ $stop_stage -ge 8 ]; then
  ...
  mkdir -p data_cif/lm
  if [ ! -f data_cif/lm/G_3_gram.fst.txt ]; then
    # It is used in building HLG
    python3 -m kaldilm \
      --read-symbol-table="data_cif/lang_phone/words.txt" \
      --disambig-symbol='#0' \
      --max-order=3 \
      $dl_dir/lm/3-gram.pruned.1e-7.arpa > data_cif/lm/G_3_gram.fst.txt
  fi

  if [ ! -f data_cif/lm/G_4_gram.fst.txt ]; then
    # It is used for LM rescoring
    python3 -m kaldilm \
      --read-symbol-table="data_cif/lang_phone/words.txt" \
      --disambig-symbol='#0' \
      --max-order=4 \
      $dl_dir/lm/4-gram.arpa > data_cif/lm/G_4_gram.fst.txt
  fi
fi

if [ $stage -le 9 ] && [ $stop_stage -ge 9 ]; then
  log "Stage 9: Compile HLG"
  ./local/compile_hlg.py --lang-dir data_cif/lang_phone

  # Note If ./local/compile_hlg.py throws OOM,
  # please switch to the following command
  #
  # ./local/compile_hlg_using_openfst.py --lang-dir data_cif/lang_phone
...
if [ $stage -le 10 ] && [ $stop_stage -ge 10 ]; then
  ...
  ./local/compile_lg.py --lang-dir data_cif/lang_phone
...
if [ $stage -le 11 ] && [ $stop_stage -ge 11 ]; then
  ...
  for vocab_size in ${vocab_sizes[@]}; do
    ...
    lang_dir=data_cif/lang_bpe_${vocab_size}
    out_dir=data_cif/lm_training_bpe_${vocab_size}
    mkdir -p $out_dir
