...
def mid_greedy_search_batch(
    ...
    Returns:
      ...
      decoded result and corresponding timestamps.
    """
    assert mid_encoder_out.ndim == 3
    assert mid_encoder_out.size(0) >= 1, mid_encoder_out.size(0)

    packed_encoder_out = torch.nn.utils.rnn.pack_padded_sequence(
        input=mid_encoder_out,
        lengths=encoder_out_lens.cpu(),
        batch_first=True,
        enforce_sorted=False,
    )
    ...
    decoder_out = model.decoder(decoder_input, need_pad=False)
    decoder_out = model.mid_joiner.decoder_proj(decoder_out)
    # decoder_out: (N, 1, decoder_out_dim)

    encoder_out = model.mid_joiner.encoder_proj(packed_encoder_out.data)
...
def tie_greedy_search_batch(
    ...
    packed_mid_encoder_out = torch.nn.utils.rnn.pack_padded_sequence(
        input=mid_encoder_out,
        lengths=encoder_out_lens.cpu(),
        batch_first=True,
        enforce_sorted=False,
    ...
    batch_size_list = packed_mid_encoder_out.batch_sizes.tolist()
    N = mid_encoder_out.size(0)
    assert torch.all(encoder_out_lens > 0), encoder_out_lens
    assert N == batch_size_list[0], (N, batch_size_list)
    ...
    decoder_out = model.decoder(decoder_input, need_pad=False)
    mid_decoder_out = model.mid_joiner.decoder_proj(decoder_out)
    # decoder_out: (N, 1, decoder_out_dim)
    decoder_out = model.joiner.decoder_proj(decoder_out)
    # decoder_out: (N, 1, decoder_out_dim)

    mid_encoder_out = model.mid_joiner.encoder_proj(packed_mid_encoder_out.data)
    encoder_out = model.joiner.encoder_proj(packed_encoder_out.data)
    ...
    for t, batch_size in enumerate(batch_size_list):
        ...
        current_mid_encoder_out = mid_encoder_out.data[start:end]
        current_mid_encoder_out = current_mid_encoder_out.unsqueeze(1).unsqueeze(1)
        # current_encoder_out's shape: (batch_size, 1, 1, encoder_out_dim)
        ...
        mid_decoder_out = mid_decoder_out[:batch_size]
        decoder_out = decoder_out[:batch_size]

        mid_logits = model.mid_joiner(
            current_mid_encoder_out, mid_decoder_out.unsqueeze(1), project_input=False
        )
        logits = model.mid_joiner(
            current_encoder_out, decoder_out.unsqueeze(1), project_input=False
        )
        ...
        logits = logits.squeeze(1).squeeze(1)  # (batch_size, vocab_size)
        mid_logits = mid_logits.squeeze(1).squeeze(1)  # (batch_size, vocab_size)
        logits += mid_logits
        assert logits.ndim == 2, logits.shape
    ...
    ans_timestamps = []
    ans_scores = []
    unsorted_indices = packed_mid_encoder_out.unsorted_indices.tolist()
    for i in range(N):
        ans.append(sorted_ans[unsorted_indices[i]])
        ans_timestamps.append(timestamps[unsorted_indices[i]])
        ans_scores.append(scores[unsorted_indices[i]])
