...
class AsrModel(nn.Module):
    ...
    def forward_transducer(
        ...
        attn_weights = self.joiner.attn_weights(
            key=am.permute(1, 0, 2),  # (N, T, C) -> (T, N, C)
            query=lm.permute(1, 0, 2),  # (N, T, C) -> (T, N, C)
            # pos_emb=self.joiner.pos_encode(decoder_out.permute(1, 0, 2)), NOTE: deprecated
        ...
        cross_attn = cross_attn + lm
        cross_attn = self.joiner.ff(cross_attn) + cross_attn
        cross_attn = self.joiner.ce_out(cross_attn)
