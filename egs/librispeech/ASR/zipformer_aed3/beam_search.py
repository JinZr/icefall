...
def deprecated_greedy_search_batch_for_cross_attn(
    ...
    for t in range(T):
        ...
        if emitted:
            ...
            decoder_out = model.decoder(decoder_input, need_pad=False)
            decoder_out = model.joiner.decoder_proj(decoder_out)
            accu_decoder_out.append(decoder_out)

    # attn_weights = model.joiner.attn_weights(
    #     key=encoder_out.permute(1, 0, 2),  # (N, T, C) -> (T, N, C)
    #     query=torch.stack(accu_decoder_out)
    #     .squeeze(2),  # (N, T, C) -> (T, N, C)
    #     key_padding_mask=make_pad_mask(lengths=encoder_out_lens),
    # )
    # cross_attn = model.joiner.cross_attn(
    #     x=encoder_out.permute(1, 0, 2),
    #     attn_weights=attn_weights,
    # ).permute(
    #     1, 0, 2
    # )  # (N, T, C)
    # logits_all = torch.stack(accu_logits).squeeze(2)
    ans = [h[context_size:] for h in hyps]
    return ans
