...
class JETSGenerator(torch.nn.Module):
    ...
    def forward(
        ...
        # upsampling
        h_masks = ~make_pad_mask(feats_lengths).to(hs.device)
        d_masks = ~make_pad_mask(text_lengths).to(ds.device)
    ...
    def inference(
        ...
        if feats_lengths is not None:
            ...
            h_masks = None
        d_masks = ~make_pad_mask(text_lengths).to(d_outs.device)
    ...
    def _source_mask(self, ilens: torch.Tensor) -> torch.Tensor:
        ...
        """
        x_masks = ~make_pad_mask(ilens).to(next(self.parameters()).device)
