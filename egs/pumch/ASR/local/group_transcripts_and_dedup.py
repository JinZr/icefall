...
def main() -> None:
    ...
    # If a given index has more than five *identical* transcripts, keep only
    # the single most common transcript to avoid redundant clutter.
    for idx, transcripts in list(grouped.items()):
        counts = Counter(transcripts)
        # Determine whether any transcript appears more than five times
        if any(v > 5 for v in counts.values()):
            most_common_transcript = counts.most_common(1)[0][0]
            grouped[idx] = [most_common_transcript]
