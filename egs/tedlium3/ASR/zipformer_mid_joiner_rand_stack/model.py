...
class AsrModel(nn.Module):
    def __init__(
        ...
        mid_joiners: Optional[nn.Module] = None,
        joiner: Optional[nn.Module] = None,
        mid_encoder_dims: List[int] = [384, 384, 384, 384],
        encoder_dim: int = 384,
        decoder_dim: int = 512,
        vocab_size: int = 500,
        ...
        self.mid_encoder_dims = mid_encoder_dims
        ...
        if use_transducer:
            ...
            self.mid_simple_am_projs = [
                ScaledLinear(mid_encoder_dim, vocab_size, initial_scale=0.25).cuda()
                for mid_encoder_dim in mid_encoder_dims
            ]
    ...
    def forward_encoder(
        self, x: torch.Tensor, x_lens: torch.Tensor, selected_index: int
        ...
        mid_encoder_out, encoder_out, encoder_out_lens = self.encoder(
            x, x_lens, selected_index, src_key_padding_mask
        )
    ...
    def forward_mid_transducer(
        ...
        am = self.mid_simple_am_projs[selected_index](mid_encoder_out)
        ...
        # lm_pruned : [B, T, prune_range, decoder_dim]
        am_pruned, lm_pruned = k2.do_rnnt_pruning(
            am=self.mid_joiners[selected_index].encoder_proj(mid_encoder_out),
            lm=self.mid_joiners[selected_index].decoder_proj(decoder_out),
            ranges=ranges,
        )
        ...
        # project_input=False since we applied the decoder's input projections
        # prior to do_rnnt_pruning (this is an optimization for speed).
        logits = self.mid_joiners[selected_index](
    ...
    def forward(
        ...
        # Compute encoder outputs
        selected_index = random.randint(1, 4)
        mid_encoder_out, encoder_out, encoder_out_lens = self.forward_encoder(
            x, x_lens, selected_index=selected_index
        )
